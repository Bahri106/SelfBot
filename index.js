...
unregister = false
kentod = "Command tidak di temukan"
...
//VOTING FUNCTION 
     ...
     if (body.startsWith(`${prefix}owner`)){
let ini_list = []
...
"displayName": `Developer ${fake}`,
"contacts": ini_list 
}, 'contactsArrayMessage', { contextInfo:forward})
}
...
//ANTI SPAM
        ...
        return sendSticker(spam1, sticker)
					}
...
//Jika ada yg kirim pesan "Asalamualaikun" botz akan respon
            	...
            	if (isOwner) return 
                  ...
                  sendSticker1(kumsalam, sticker)
                  setTimeout( () => {
                  sendSticker1(ketikmenu, sticker)
                   },5000)
                  }
...
gbutsan = [
 ...
 gbuttonan = {
imageMessage: mhan.message.imageMessage,
    contentText: help.menu(chatBot, settings, nomerOwner, forwarding, fake, autoread, autovn, typing, Antibuggc,antibugtroli,Toxic, isAntihidetag, isKickarea, isAntiLink, AntiVirtex, isAntiviewonce, cmhit, pushname, ucapanWaktu, prefix, banChats, time2, week, weton, calender),
...
case 'self':
           ...
           case 'welcome':
                 ...
                 case 'unregister':
					       if (!isOwner) return sendSticker(gaboleh, sticker)
					if ((args[0]) === 'on') {
					if (unregister === true) return jawab('*Udah aktif kak*')
                    unregister = true
                    let ih =`Mode unregister telah aktif kak`
                    xdev.sendMessage(from, ih, text, { quoted: ftext, contextInfo: forward})
					} else if ((args[0]) === 'off') {
					 if (unregister === false) return jawab('*Udah off kak*')
                     unregister = false
                     let eh =`Mode unregister telah di matikan`
                     xdev.sendMessage(from, eh, text, { quoted: fmenu1, contextInfo: forward})
					}else if (!q) {
                      sendButMessage(from, `MODE AUTO VOICE NOTE`, `Silahkan pilih salah satu`, [
                      {buttonId: `${prefix}unregister on`, buttonText: {displayText: `on`,},type: 1},
                      {buttonId: `${prefix}unregister off`, buttonText: { displayText: `off`,},type: 1}]);
                    }
                 break;
...
sendMediaURL(from,`${res.result.link}`,`${res.result.desc}`)
    ...
    case 'join':
				if (!isOwner && !dev.key.fromMe) return reply(mess.only.ownerB)
				 if (args.length < 1) return reply('Masukan kode invite?')
					xdev.query({
...
default:
                        ...
                        if(unregister === true) return  xdev.sendMessage(from, kentod, text, {quoted : dev})
